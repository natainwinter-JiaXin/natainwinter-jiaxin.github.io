const siteConfig = {
  ...
  portfolio: [
    ...
    },
    {
      title: "关系练习",
      meta: "TXT",
      file: "assets/portfolio/关系练习.txt",
      type: "txt",
      icon: "assets/3d icon/3D Glass Bookmark Icon.png",
    },
    {
...
const applyConfig = () => {
  ...
  if (introTitle) introTitle.textContent = siteConfig.intro.title;

  const introSubtitle = qs("#introSubtitle");
  if (introSubtitle) introSubtitle.textContent = siteConfig.intro.subtitle;

  const enterBtn = qs("#enterBtn");
  if (enterBtn) enterBtn.textContent = siteConfig.intro.enterLabel;

  const mainName = qs("#mainName");
  if (mainName) mainName.textContent = siteConfig.main.name;

  const mainSignature = qs("#mainSignature");
  if (mainSignature) mainSignature.textContent = siteConfig.main.signature;

  const avatar = qs("#avatar");
  if (avatar) avatar.textContent = siteConfig.main.avatarText;
...
const initScrollProgress = () => {
  ...
  const scrollTrack = qs("#verticalScrollBar");
  const scrollThumb = qs("#verticalScrollThumb");
  if (!progressBar) return;

  const getScrollElement = () => {
    const portfolioElement = qs(".portfolio");
    if (portfolioElement && portfolioElement.scrollHeight > portfolioElement.clientHeight) {
      return portfolioElement;
    }
    return window;
  };

  const updateProgress = (scrollElement = getScrollElement()) => {
    ...
    if (scrollTrack && scrollThumb) {
      ...
      const thumbTop = docHeight > 0 ? (scrollTop / docHeight) * maxThumbTop : 0;
      scrollThumb.style.height = `${thumbHeight}px`;
      scrollThumb.style.transform = `translateY(${thumbTop}px)`;
      scrollTrack.classList.toggle("hidden", docHeight <= 0);
    }
  };

  const scrollElement = getScrollElement();
  if (scrollElement === window) {
    window.addEventListener("scroll", () => updateProgress(window), { passive: true });
  } else {
    scrollElement.addEventListener("scroll", () => updateProgress(scrollElement), { passive: true });
  }

  const observer = new MutationObserver(() => updateProgress(getScrollElement()));
  observer.observe(document.body, { attributes: true, attributeFilter: ["class"] });

  updateProgress(getScrollElement());
};

initScrollProgress();

// --- Portfolio viewer: merged from assets/portfolio-viewer.js ---
(() => {

  const syncViewportHeight_viewer = () => {
    document.documentElement.style.setProperty("--app-height", `${window.innerHeight}px`);
  };

  const stripRtf_viewer = (rtf) => {
    let text = rtf;
  ...
  const cacheBustedUrl_viewer = (path) => {
    const url = new URL(encodeURI(path), window.location.href);
    url.searchParams.set("_", Date.now());
    return url.href;
  };

  const renderText_viewer = (bodyEl, content) => {
  ...
  const initViewer = async () => {
    syncViewportHeight_viewer();
    window.addEventListener("resize", syncViewportHeight_viewer);
    window.addEventListener("orientationchange", syncViewportHeight_viewer);
    window.scrollTo(0, 0);
    ...
    const fileUrl = cacheBustedUrl_viewer(file);
    const prefersNativePdfViewer = window.matchMedia("(pointer: coarse)").matches || window.innerWidth <= 600;
    ...
    if (type === "pdf") {
      ...
      try {
        ...
        renderText_viewer(bodyEl, stripRtf_viewer(rtf));
      } catch (error) {
        const frame = document.createElement("iframe");
        frame.className = "reader-embed";
        frame.src = fileUrl;
